...
export function ComposeInput({
  ...
  const handleCancelOrEscape = useCallback(() => {
    ...
    if (text.trim() || mediaFiles.length > 0 || linkCard) {
      ...
      // If no content, just close without dialog
      // (add forceClose if you have a global close handler)
    }
  }, [showMentionSuggestions, showHashtagSuggestions, text, mediaFiles.length, linkCard])
